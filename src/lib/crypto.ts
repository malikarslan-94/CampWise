 ...
 */
export function encrypt(payload: unknown, keyHex: string): string {
 ...
 */
export function decrypt<T = unknown>(blob: string, keyHex: string): T {
  const [ivHex, cipherBase64] = blob.split(':');
  if (!ivHex || !cipherBase64) throw new Error('Invalid encrypted blob format');
