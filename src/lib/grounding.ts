...
export async function groundPlan(
  plan: CanonicalPlan,
  originalRequest: CanonicalSearchPlansRequest,
  ctx: ResolvedContext,
): Promise<boolean> {
  ...
  try {
    const verifyReq: CanonicalSearchPlansRequest = {
    ...
    if (Math.abs(match.price - plan.price) > PRICE_TOLERANCE) {
      logger.warn(
        { planCode: plan.planCode, event: 'grounding_mismatch', reason: 'price_changed', originalPrice: plan.price, newPrice: match.price },
        'grounding_mismatch',
      );
      return false;
    }
 ...
 */
export async function groundPricingResult(
  originalResult: CanonicalPricingResponse,
  ...
  try {
    ...
    if (
      originalResult.totalAmount !== undefined &&
